...
def send_message(socket, address, message):
    store_message.append(f"Server 2 ke {address}: {message}")
...
def send_file(socket, server_address, file_format):
    if file_format == 3:
        file_path = "server2/server2.pdf"
    elif file_format == 4:
        file_path = "server2/server2.jpg"
    elif file_format == 5:
        file_path = "server2/server2.mp3"
    elif file_format == 6:
        file_path = "server2/server2.mp4"
...
# Fungsi utama yang menjalankan logika aplikasi. Fungsi ini mengatur thread untuk penerimaan pesan dan meminta input pengguna
# untuk memilih metode pengiriman (unicast, multicast, broadcast) serta mode pesan
def main():
    threading_slot()
