...
def send_message(socket, address, message):
    store_message.append(f"Server 10 ke {address}: {message}")
    socket.sendto(message.encode("utf-8"), address)


def send_file(socket, server_address, file_format):
    if file_format == 3:
        file_path = "server10/server10.pdf"
    elif file_format == 4:
        file_path = "server10/server10.jpg"
    elif file_format == 5:
        file_path = "server10/server10.mp3"
    elif file_format == 6:
        file_path = "server10/server10.mp4"
...
def recieve_file(socket, file_info):
    ...
    destination_folder = "download_server10"
