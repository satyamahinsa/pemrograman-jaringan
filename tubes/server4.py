...
def send_message(socket, address, message):
    store_message.append(f"Server 4 ke {address}: {message}")
    socket.sendto(message.encode("utf-8"), address)


def send_file(socket, server_address, file_format):
    if file_format == 3:
        file_path = "server4/server4.pdf"
    elif file_format == 4:
        file_path = "server4/server4.jpg"
    elif file_format == 5:
        file_path = "server4/server4.mp3"
    elif file_format == 6:
        file_path = "server4/server4.mp4"
